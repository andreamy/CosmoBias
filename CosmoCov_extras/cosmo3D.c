...
double p_lin(double k,double a)
{
  ...
  if (strcmp(pdeltaparams.runmode,"CLASS")==0 || strcmp(pdeltaparams.runmode,"class")==0) {
      // printf("\n\n return p_class without problems.\n\n"); // only used in NG mode
      return p_class(k, a, 0, &status);
  };
...
{ 
  static int P_type = -1;
  if (P_type == -1){
    if (strcmp(pdeltaparams.runmode,"Halofit")==0) P_type = 0; //by default
    if (strcmp(pdeltaparams.runmode,"halofit")==0) P_type = 0;row:
    ...
    if (strcmp(pdeltaparams.runmode,"class")==0){
        P_type = 4;
        //printf("\n\n p_type set to 4, GOOD.\n\n");
    }
  ...
  switch (P_type){
    ...
    case 4: pdelta=p_class(k_NL,a,1, &status); break; //exits here if case = 4
...
/*========CLASS functions from CosmoLike Light - https://github.com/CosmoLike/cosmolike_light ======*/
double p_class(double k_coverh0,double a, int NL, int *status){
    //printf("\n\n p_class is entered, GOOD.\n\n");
    static cosmopara C;
    ...
    if (recompute_cosmo3D(C)){
        ...
        *status = run_class(&fc,&ba,&th,&pt,&tr,&pm,&hr,&fo,&le, &sd);
        ...
        //if cosmology.a_s > 0 (always true if we pass a value in class, then no need for else statement
        else{
        ...
        if (*status ==0){
            for (i=0; i<Ntable.N_a; i++, aa +=da) {
                ...
                for (j=0; j<Ntable.N_k_nlin; j++, klog += dk) {
                    ...
                    s = fourier_pk_at_k_and_z(&ba, &pm, &fo, pk_linear, k_class, fmax(1./aa-1.,0.), fo.index_pk_m, &Pk, &ic);
                    table_P_L[i][j] = log(Pk) +norm;
                    s = fourier_pk_at_k_and_z(&ba, &pm, &fo, pk_nonlinear, k_class, fmax(1./aa-1.,0.), fo.index_pk_m, &Pk, &ic);
...
//     printf("passing A_s=%e directly\n",cosmology.A_s);
        ...
        /*if (status == 0){ //commented out to avoid warning, shouldn't enter in else above anyhow
    ...
    //strcpy(fc->value[1],"HMcode"); //to use HMcode within CLASS
    strcpy(fc->value[1],"Halofit"); //to use Halofit within CLASS
    //strcpy(fc->name[2],"feedback model");
    //strcpy(fc->value[2],"owls_dblim");

    //strcpy(fc->name[17],"eta_0");
    //sprintf(fc->value[17],"%e",0.644);
    // feedback model parameter, obtained with η0 = 0.98−0.12*2.8
    // for more info see methodology paper from k1000, page 4 first paragraph
    return status;
}
