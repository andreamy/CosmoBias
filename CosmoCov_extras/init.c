...
void set_angular_binning(double *thetamin, double *dtheta){
  double *thetamax;
  double *log_theta_left;
  log_theta_left = create_double_vector(0, like.Ntheta);//NEW
  ...
  if(covparams.lin_bins){ /* linear binning */
    ...
    for(int i=0; i<like.Ntheta; i++){
      ...
      thetamin[i]=exp(log_theta_left[i]+logdt*0.5); //NEW, now theta center, not lower bound
      thetamax[i]=exp(log(like.vtmin)+(i+1.0)*logdt+logdt*0.5);//NEW, now theta center max
      //thetamin[i]=exp(log(like.vtmin)+(i+0.0)*logdt);//OLD, lower bound of theta(left bin)
      //thetamax[i]=exp(log(like.vtmin)+(i+1.0)*logdt);// OLD
      dtheta[i]=thetamax[i]-thetamin[i];
      //printf(" thetamin[%d] = %le, thetamax[%d] = %le, dtheta[%d] = %le\n", i, thetamin[i], i, thetamax[i], i , dtheta[i]);
    }
  }
  thetamin[like.Ntheta] = thetamax[like.Ntheta-1];
  //printf("\n thetamin[%d] = %le, equal to thetamax[%d] \n", like.Ntheta, thetamin[like.Ntheta], like.Ntheta-1);
...
void init_source_sample(char *multihisto_file, int Ntomo)
{
  ...
  tomo.shear_Nbin = Ntomo; // source_tomobins (5 for kids)
...
void set_cov_parameters(char *covparamfile, int output)
{
  ...
  while(fgets(line, 256, input) != NULL)
  {
    ...
    else if(strcmp(name, "simulation")==0)
    {
      sprintf(covparams.simulation,"%s", val);
        if(output==1)
        {
            printf("simulation %s \n",covparams.simulation);
        }
        continue;
    }
