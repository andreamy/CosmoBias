...
void run_cov_shear_shear_real_binned(char *SIMULATION, char *OUTFILE, char *PATH, double *t, double *theta, double *dtheta, int Ntheta, int n1, int n2, int pm1, int pm2, int start);
...
void run_cov_shear_shear_real_binned(char *SIMULATION, char *OUTFILE, char *PATH, double *t, double *theta, double *dtheta, int Ntheta, int n1, int n2, int pm1, int pm2, int start)
{
  ...
  //MODIFICATION FROM ANDREA - TO PRINT XIPM FOR EVERY BIN AND THETA
  FILE *xipm1, *xipm2;
  char filenamepm1[300], filenamepm2[300];
  double *xip, *xim;

  sprintf(filenamepm1, "./xipm/%s/xipm_kids_%s_%d-%d.txt", SIMULATION, SIMULATION, z1, z2);
  sprintf(filenamepm2, "./xipm/%s/xipm_kids_%s_%d-%d.txt", SIMULATION, SIMULATION, z3, z4);
  xipm1 = fopen(filenamepm1, "w");
  xipm2 = fopen(filenamepm2, "w");
  xip = create_double_vector(0, like.Ntheta - 1);
  xim = create_double_vector(0, like.Ntheta - 1);
